...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$last_row_user = new StdClass;
        $last_row_user=$this->db->select('id')->order_by('id',"desc")->limit(1)->get('users')->row();

        if (empty($last_row_user->id)) {$last_row_id_user = 0;} else {$last_row_id_user = $last_row_user->id;}

        $user_data['id'] = $last_row_id_user+1;
		...
		/* Create account for all the users within */
		//To get the most recent ID
		$last_row = new StdClass;
        $last_row=$this->db->select('account_id')->order_by('account_id',"desc")->limit(1)->get('accounts')->row();


        if (empty($last_row->account_id)) {$last_row_id = 0;} else {$last_row_id = $last_row->account_id;}

        if (!empty($additional_data['company'])) {
        	$account_name = $additional_data['company'];
        } else {
        	$account_name = $additional_data['first_name'].' '.$additional_data['last_name'];
        }
        $account['account_id'] = $last_row_id+1;
        $account['name'] = $account_name;

        $relate_account['user_id'] = $user_data['id'];
        $relate_account['account_id'] = $last_row_id+1;


        $this->db->insert('accounts',$account);
        $this->db->insert('user_accounts',$relate_account);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, job_title')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function account_id_find($user_id)
	{
		$query = $this->db->query("SELECT * from user_accounts WHERE user_id = ".$user_id);

		$return = $query->result();

		return $return[0]->account_id;
	}
 ...
 $last_row=$this->db->select('id')->order_by('id',"desc")->limit(1)->get('groups')->row();

 //Increment last ID plus 1
	  $group_id = $last_row->id+1;
		$data = array('id'=>$group_id, 'name'=>$group_name,'description'=>$group_description);
